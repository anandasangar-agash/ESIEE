...
public final class JExpress {
  ...
  record Event(String id, String when, String what, String where) {}

  record Agenda(List<Event> events) {
    Agenda() {
      this(new ArrayList<>());
    }
    void addEvent(String when, String what, String where) {
      events.add(new Event("" + UUID.randomUUID(), when, what, where));
    }
  }

  static void main() {
  	var agenda = new Agenda();
    agenda.addEvent("2025-07-20T10:00", "Agile Meeting", "Google Meet");
    agenda.addEvent("2025-07-22T14:30", "PM Call", "Skype");
    agenda.addEvent("2025-07-20T09:00", "Kanban Update", "John's Desk");

    var app = express();
    app.use(staticFiles(Path.of(".")));
    app.get("/api/event", (req, res) -> {
      sleep(2_000);  // simulate heavy load
    	System.out.println("get all events " + agenda.events);
    	res.json(agenda.events);
    });
    app.post("/api/event", (req, res) -> {
      sleep(2_000);  // simulate heavy load
      var simpleEvent = req.bodyObject();
      System.out.println("add event " + simpleEvent);
      agenda.addEvent((String) simpleEvent.get("when"), (String) simpleEvent.get("what"), (String) simpleEvent.get("where"));
    	res.json(agenda.events.getLast());
    });
    app.put("/api/event", (req, res) -> {
      sleep(2_000);  // simulate heavy load
      var event = req.bodyObject();
      System.out.println("update event " + event);
      var index = IntStream.range(0, agenda.events.size())
          .filter(i -> agenda.events.get(i).id.equals(event.get("id")))
          .findFirst()
          .orElse(-1);
      if (index == -1) {
        res.status(404).send("<html><h2>Not Found</h2></html>");
        return;
      }
      agenda.events.set(index, new Event((String) event.get("id"), (String) event.get("when"), (String) event.get("what"), (String) event.get("where")));
      res.json(event);
    });
    app.delete("/api/event/:id", (req, res) -> {
      ...
      agenda.events.removeIf(e -> id.equals(e.id));
      res.status(200).send("<html><h2>Ok</h2></html>");
    });

    app.listen(8080);
    
    IO.println("start on http://localhost:8080/agenda.html");
  }
}
